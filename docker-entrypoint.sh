...
if [ "$1" = 'tomcat' ]; then
  ...
  export CATALINA_OPTS=$CATALINA_OPTS' -DMAIL_HOST=localhost -DMAIL_PORT=2525 -DMAIL_AUTHENTICATE=false -DMAIL_SSL_ON_CONNECT=false'
  ...
  export CATALINA_OPTS=$CATALINA_OPTS$POSTGRES_ENV_POSTGRES_DB
